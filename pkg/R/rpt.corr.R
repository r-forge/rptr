rpt.corr <- function(y, groups, CI=0.95, nboot=1000, npermut=1000) {
	...
	R.permut <- replicate(npermut, permut(y1, y2, k), simplify=TRUE)
	P        <- sum(R.permut >= R)/npermut
	# return of results
	res <- list(datatype="Gaussian", method="corr", CI=CI, R=R, se=se, CI.R=CI.R, P=P, R.permut=R.permut) 
	class(res) <- "rpt"
	return(res) 
}