rpt <- function(y, groups, 
	...
	if(datatype=="Gaussian") {
		...
		if (method=="REML")  return(rpt.remlLMM(y, groups, CI, npermut, nboot))
		if (method=="MCMC")  return(rpt.mcmcLMM(y, groups, CI))
		if (method=="ANOVA") return(rpt.aov(y, groups, CI, npermut))	
		if (method=="corr")  return(rpt.corr(y, groups, CI, npermut, nboot)) 
	}
	if(datatype=="binomial" | datatype=="proportion") {
		...
		if (method=="GLMM.multi") return(rpt.binomGLMM.multi(y, groups, link, CI, npermut, nboot))
		if (method=="GLMM.add") return(rpt.binomGLMM.add(y, groups, CI))
	}
	if(datatype=="count") {
		if(length(method)>1) {
		...
		if (method=="GLMM.multi") return(rpt.poisGLMM.multi(y, groups, link, CI, npermut, nboot)) 
		if (method=="GLMM.add")   return(rpt.poisGLMM.add(y, groups, CI)) 
	} 
}