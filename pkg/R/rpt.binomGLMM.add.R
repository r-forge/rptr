rpt.binomGLMM.add <- function(y, groups, CI=0.95, prior=NULL) {
	# initial checks
	if (is.null(dim(y))) 
		y      <- cbind(y, 1-y)
	...
	if(all(n==1)) {
		...
		mod    <- MCMCglmm(cbind(m, nm) ~ 1, random= ~ groups, data=data.frame(m=y[,1],nm=y[,2],groups=groups), prior=prior, family="multinomial2", verbose=FALSE)
	}
	...
	P.org      <- NA
	# return of results
	res        <- list(datatype="binomial", method="MCMC", CI=CI,
